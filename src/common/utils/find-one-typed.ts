...
export async function findOneOrFailTyped<TReturnType>(
  ...
  const standardErrorMessage = `${repository.metadata.name} not found with specified field:${searchFieldName}, value: ${value}`;

  const entity = await repository.findOne(options);

  if (!entity) {
    throw new HttpException(standardErrorMessage, HttpStatus.NOT_FOUND);
  }
