...
import { IQueueData, IQueueDataBulk, IQueueSettings } from 'src/libs/queues/queue/common/interfaces';
...
export class QueueManagementService implements OnModuleInit, OnModuleDestroy {
  ...
  public onModuleInit(): void {
    for (const queueEnum of Object.values(EQueueType)) {
      const { queueName, queueOptions } = this.getQueueSettings(queueEnum);
   ...
   *
   */
  private getQueueSettings(queueEnum: EQueueType): IQueueSettings {
    const DEFAULT_JOB_OPTIONS = {
      ...
      removeOnFail: {
        age: NUMBER_OF_MINUTES_IN_FIFTEEN_DAYS * NUMBER_OF_SECONDS_IN_MINUTE,
        count: 200,
      },
  ...
  public async addBulk(jobData: IQueueDataBulk, opts?: BulkJobOptions): Promise<void> {
    ...
    const jobs = jobItems.map((jobItem) => ({
      name: jobItem.jobName,
      data: { jobName: jobItem.jobName, payload: jobItem.payload },
      opts: opts,
    }));
