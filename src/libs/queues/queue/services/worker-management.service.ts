 ...
 * job processing delegation through the {@link QueueConsumerBridgeService}, and
...
export class WorkerManagementService implements OnModuleInit, OnModuleDestroy {
  ...
  constructor(
    ...
    private readonly queueConsumerService: QueueConsumerBridgeService,
  ) {}
   ...
   * and uses {@link QueueConsumerBridgeService.processJob} as its job processing function.
   *
  ...
  public onModuleInit(): void {
    for (const queueEnum of Object.values(EQueueType)) {
      const { queueName, workerOptions } = this.getWorkerSettings(queueEnum);
      this.lokiLogger.debug(`Initializing Worker for queue [${queueEnum}] with name "${queueName}"`);

      const worker = new Worker(
        queueName,
        async (job: Job<IQueueJobType>) => {
          return await this.queueConsumerService.processJob(queueEnum, job);
        },
        {
   ...
   *
   */
  private getWorkerSettings(queueEnum: EQueueType): IWorkerSettings {
